...
import { TerminalOutput } from "@/components/TerminalOutput"; // Adjust path if needed
...
export default function ScanPage() {
    ...
    const handleScan = async () => {
        ...
        try {
            ...
            // Simulate/Show results (In a real app, we'd poll or use SSE)
            setLogs(prev => [
                ...prev,
            ...
            <div className="grid gap-6 md:grid-cols-[1fr_300px]">
                ...
                <div className="space-y-6">
                    <div className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-md">
                        <label className="block text-sm font-medium text-muted-foreground mb-2">Repository URL</label>
                        <input
                            type="text"
                            placeholder="https://github.com/username/repo"
                            value={repoUrl}
                            onChange={(e) => setRepoUrl(e.target.value)}
                            className="w-full rounded-lg border border-white/10 bg-black/50 px-4 py-3 font-mono text-sm text-white placeholder-white/20 focus:border-green-500/50 focus:outline-none focus:ring-1 focus:ring-green-500/50"
                        />
                        ...
                        <div className="mt-6 flex items-center justify-between">
                            ...
                            >
                                ...
                                <div className="text-sm">
                                    ...
                                    <span className="text-xs text-muted-foreground">Uses Gemini Pro (Slower, more accurate)</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <button
                            ...
                            className={cn(
                                "mt-6 flex w-full items-center justify-center gap-2 rounded-lg py-4 font-semibold text-black transition-all",
                                isScanning ? "bg-white/50 cursor-not-allowed" : "bg-white hover:bg-gray-200"
                            )}
                        >
                            {isScanning ? (
                    ...
                    <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-purple-500/10 to-blue-500/10 p-6">
                        ...
                        <p className="text-xs text-purple-200/70 leading-relaxed">
                            Scanner utilizes a hybrid architecture. <b>Gemini Flash</b> performs rapid triage on file signatures, while <b>Gemini Pro</b> conducts semantic analysis on high-risk vectors.
                        </p>
                    </div>
                </div>
                ...
                <div className="md:pt-0 pt-4">
                    <TerminalOutput logs={logs} className="h-full min-h-[400px]" />
                </div>
            </div>
        </div>
    );
